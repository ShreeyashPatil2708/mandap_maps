...
// Same pin shape as before, but the fill color is now a parameter so we can
// paint it green/yellow/red/maroon based on live crowd data.
...
const DEFAULT_COLOR = '#6B1E2E'; // your existing maroon
const CROWD_COLORS = { 1: '#3E8E5A', 2: '#C99A2E', 3: '#B23A3A' }; // green / gold-amber / red
  ...
  return (
    <div className="overflow-hidden rounded-card border border-maroon/[0.08]">
      ...
      >
        ...
        />
        {pins.map((g) => {
          const level = crowd[g.id]?.level;
          const icon = makePinIcon(CROWD_COLORS[level] || DEFAULT_COLOR);
          return (
